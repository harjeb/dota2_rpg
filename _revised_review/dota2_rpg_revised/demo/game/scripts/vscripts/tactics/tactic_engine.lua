...
function TacticEngine:Debug(unit, event, detail)
    if self.on_debug ~= nil then
        self.on_debug(unit, event, detail or {})
    end
end
...
function TacticEngine:EvaluateUnit(unit, state, current_time)
    ...
    if state.chase ~= nil then
        local max_priority = math.max(0, (state.chase.rule_index or 1) - 1)
        local handled = self:EvaluateRules(unit, state, ctx, rules, 1, max_priority)
        if handled then
            state.chase = nil
            return
        end
...
function TacticEngine:EvaluateRules(unit, state, ctx, rules, first_index, last_index)
