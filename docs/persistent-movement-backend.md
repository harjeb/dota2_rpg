...
| `movement_buff` | Optional native modifier name; empty means no buff requirement, otherwise checked on caster with `HasModifier` |
...
The three **movement preset buttons remain** (`V2MovementPreset_shukuchi`, `_trample`, `_gyroshell`). They overwrite only movement settings in the draft, preserving team, conditions and priorities. **关联原生增益** (`V2MovementBuffSelect`) offers No associated buff (new-draft default) or Require a chosen buff. The latter shows the status dropdown (`V2MovementStatusSelect`), which selects an observed/declared native status and stores `movement_buff`; switching to no buff saves an empty field and hides that dropdown. There is no raw modifier textbox. Condition status parameters likewise use status dropdowns. Unknown saved names remain visible as unavailable entries with a reason and must be replaced with a valid selection. Selecting a status changes only the buff, while a movement preset fills the whole movement configuration.
...
- Without either buff or trigger, one continuous episode of satisfied use conditions arms one session. Timeout/path completion does not re-arm it; conditions must first fail and then pass again. Always-true conditions run once per battle. This keeps the authored duration meaningful rather than restarting movement each frame. Targets still must pass the existing selector before consuming the gate.
- With a buff but no trigger ability, one buff episode arms one session. Termination cannot immediately restart the same episode. Observing buff absence re-arms the next episode; changing use conditions alone does not reset this existing buff gate.
- With a trigger ability, each rule baselines the observer's cast count on its first evaluation. A cast preceding that baseline is deliberately not eligible. New native cast observations can arm a session while the optional associated buff is present, or without any buff when none is configured; issued-order history is never sufficient. New entities/stages start fresh histories. A buff loss clears pending readiness; delayed or entirely unobserved buff episodes are not reconstructed.
- Active movement suppresses ordinary attacks/fallback and native idle acquisition. Skill/item rules on either side of the movement row are evaluated in authored order and may cast from the current position, without replacing the movement path with pursuit or ability positioning. Movement yields its order during the pending native cast, cast point, channel or sustained cast, then resumes the same session; the original deadline keeps running. A pending order is bounded by the existing two-second native-observation timeout. Buff loss, expiry and target/use filters still end the session while it is paused, without a cleanup STOP cancelling the borrowed cast. Reviewed channel-release actions remain reachable.
- `movement_interruptible=true` additionally lets a higher-priority non-attack rule end the movement session early (the earlier behavior); with it off, skills/items only borrow the order. Native acquisition remains disabled until session release, and is restored during battle while preparation stays passive. Tiny's auxiliary tree cast observes the same acquisition-suppression state.
...
Attack-range positioning aims inside the live range by the configured tolerance; its accepted upper bound is the actual attack range. It first retreats radially, then tests tangential steps on either side (including a shorter corner step) and angular alternatives. Waypoints must be within attack range, open the gap when retreating, and avoid a segment cutting closer to the target. Native blocked/traversable cells are sampled along the segment, alongside `CanFindPath`; these are bounded checks, not a replacement pathfinder. A valid side waypoint is retained, and successful direction is remembered to avoid per-tick oscillation. No movement progress for 0.45 seconds causes an alternate-side attempt even when static navigation reports success. A blocked radial route is reconsidered after movement, target displacement or a two-second cooldown. If every candidate fails, positioning returns control to ordinary attack evaluation. For attack actions, all choices still occur strictly inside the original native-release window, never during windup or native casts. The native event observer survives same-handle reincarnation and is explicitly detached on stage reset.
...
- Windrun: `modifier_windrunner_windrun`, source `windrunner_windrun`.
- Focus Fire: `modifier_windrunner_focusfire`, source `windrunner_focusfire`.

Windrun and Focus Fire are dropdown entries for heroes owning those abilities, available before first cast or observation. They do not add movement-preset buttons or fabricate a buff. Their exact native status names were verified in the installed Dota VPK `resource/localization/abilities_english.txt`, entries `DOTA_Tooltip_modifier_windrunner_windrun` and `DOTA_Tooltip_modifier_windrunner_focusfire`.
...
It also covers no-buff startup, all four modes, condition-edge re-arming, unconditional once-per-battle behavior, optional cast-only triggering, clearing a saved association, and preservation of existing buff gates. `tests/ability-compatibility.test.lua` verifies Windranger's two declared buffs before observation; the real-HUD tests verify selecting, clearing, switching and reopening the optional association.
