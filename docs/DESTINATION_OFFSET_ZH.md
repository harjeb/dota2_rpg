...
- 点目标、残焰类动作以及白虎跳／原力法杖，在「条件设置 → 目标优先级」的第 1 项下拉中提供「目的地」分组；不再另设目的地下拉。
- 目的地与普通目标排序互斥：选择目的地会清除两项排序并隐藏第 2 项；切回普通排序会恢复第 2 项并清除落点距离。
- 模式列表按能力生成：点目标提供四种偏移；残焰类额外保留 `self` 与 `remnant_*` 专属落点。白虎跳／原力法杖只提供背向敌人的撤退模式。
- 选中可调距离的偏移模式时，「落点距离」紧跟目标优先级显示。
- 目的地需要敌人参照时，从符合目标筛选的单位中选最近的合法锚点。旧规则同时保存了目的地与排序时保留目的地、清除排序；目标筛选仍生效。
...
现在可选择「目标优先级第 1 项 → 目的地 → 远离目标（背向敌人）」。目标队伍为敌人，只用于计算撤退方向；
