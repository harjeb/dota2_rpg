...
- `单位明细.csv`：每个 `levels/<chXX>/enemies/<N>` 配置行的单位、数量、等级、AI、标签、普通强化、Boss 强化及装备 1–5。
- `装备明细.csv`：一件装备一行，便于按关卡、英雄、槽位或物品 ID 过滤。
...
本次导出包含 30 关、139 条敌方配置、343 条装备槽、73 种单位。工具刻意不让旧 JSON 覆盖运行时 KV。当前 `源文件差异` 中有 93 项，均为英雄 `level`：运行时 KV 是逐关等级，`levels_v07.json` 的这些英雄行仍为 30。此差异需要单独决定是否同步，不能在不经确认的情况下用其中一份覆盖另一份。
