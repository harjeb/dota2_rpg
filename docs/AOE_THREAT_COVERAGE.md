...
`tactics/techies_threats.lua` is integrated into observation, returned threats and reset. It discovers actual `npc_dota_techies_mines`, `npc_dota_thinker`, and `npc_dota_techies_minefield_sign` entities every 200 ms, revisits known sources at the engine observation cadence, and reads positions/modifiers only after a viewer confirms entity visibility. A visible bomb may be learned even when its original planting was hidden; this exposes the visible object, not the hidden cast. Evidence and exact native names are retained in `data/research/techies_native_sources_20260915.json`.
...
- Minefield Sign has a separate Scepter source observer. An actual visible Sign with `modifier_techies_minefield_sign_thinker`, positive remaining time, and a **visible caster reporting `HasScepter() == true`** produces a short-memory `trigger_radius` envelope (`minefield_state='trigger'`). A cosmetic Sign, generic thinker alone, or hidden caster inventory cannot establish upgrade status. An actual visible Sign/source thinker carrying `modifier_techies_minefield_sign_scepter` from the enemy Sign ability establishes the active `aura_radius` envelope (`minefield_state='active'`, `movement_triggered=true`), even when its caster is hidden. Its deadline is bounded by the modifier's remaining time and `minefield_duration` from the first observed activation; repeated polls cannot restart the full duration. The Sign's `lifetime` is never used as active field duration. Visible removal/destruction clears the observing team's record; hidden activation/removal never changes another team's phase or position, and unseen snapshots expire within three seconds.
- Minefield damage depends on victim movement (`scepter_move_amt` / `scepter_move_damage`), so the active circle is an avoidance envelope, not continuous damage or a predicted damage tick. The victim's `modifier_techies_minefield_sign_scepter_aura` is deliberately excluded as a spatial anchor: following the victim would move the whole field incorrectly. Nearby heroes do not synthesize activation. The native server strings confirm all three modifier names and native unit data confirms the Sign class; upstream [thinker schema](https://github.com/SteamDatabase/GameTracking-Dota2/blob/master/DumpSource2/schemas/server/CDOTA_Modifier_Techies_Minefield_Sign_Thinker.h), [Scepter schema](https://github.com/SteamDatabase/GameTracking-Dota2/blob/master/DumpSource2/schemas/server/CDOTA_Modifier_Techies_Minefield_Sign_Scepter.h), and [victim aura schema](https://github.com/SteamDatabase/GameTracking-Dota2/blob/master/DumpSource2/schemas/server/CDOTA_Modifier_Techies_Minefield_Sign_Scepter_Aura.h) distinguish a private trigger flag, source duration, and victim movement accumulation. Schemas are supporting evidence, **not native runtime proof**: Lua parent/modifier visibility and timer behavior require Dota validation. The private trigger flag is not read; if the active source is unobservable or exposes no valid timer, the full field remains uncovered. Hidden upgrade checks and exact movement-trigger damage are also unsupported.

Hidden positions never update, hidden expiry is bounded, and returned snapshots are isolated per team. Source absence is not proof of safety. Native mine trigger timers, unobservable thinkers, and exact bomb collision/movement remain unverified or uncovered. Tests cover each mine kind, free and attached Sticky phases, visible motion, hidden motion, destruction, arming, stable IDs/deadlines, missing APIs/data, and reset. `lua tests/techies-threats.test.lua` additionally checks cosmetic/Scepter Sign gating, hidden inventory reads, independent trigger/active phases per team, source versus victim anchors, modifier removal, duration caps, expiry, destruction, invalid data, and reset.
...
| `templar_assassin_psionic_trap`, `storm_spirit_static_remnant` | Spawned source identity, trigger/destruction and special lifetimes. Techies mines/barrels and observable Minefield Sign Scepter states use the separate source observer described above; they are not ordinary executed profiles. |
