...
assert(state.chase.deadline==13 and state.chase.max_distance==2000,"default chase is 3 seconds / 2000 units")
assert(engine:ContinueChase(caster,state,engine:BuildContext(caster,12.9),12.9),"default chase survives beyond old timeout")
assert(not engine:ContinueChase(caster,state,engine:BuildContext(caster,13.01),13.01) and not state.chase,"default timeout expires")
assert(attempt()); state.chase.start_position=Vector(-2000,0,0)
assert(engine:ContinueChase(caster,state,engine:BuildContext(caster,10.5),10.5),"2000 displacement is permitted")
state.chase.start_position=Vector(-2001,0,0)
assert(not engine:ContinueChase(caster,state,engine:BuildContext(caster,10.5),10.5) and not state.chase,"over 2000 displacement stops chase")
rule.chase_timeout="4.5"; assert(service:ValidateRule(0,caster,rule)); assert(attempt())
