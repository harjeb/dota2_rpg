...
local markers = {
    rpg_arena_min = Vector(-1200, -675, 128),
    rpg_arena_max = Vector(1200, 675, 128),
...
assert(#created == 15, "tree row must span the full 1350-unit divider")
assert(created[1].position.y == -651 and created[15].position.y == 651, "both ends must be covered")
...
arena:StartPrepare({})
arena:EnforceBounds()
assert(#created == 15, "prepare updates must not duplicate trees")
created[5].standing = false
arena:EnforceBounds()
assert(#created == 16 and created[5].removed, "cut preparation tree must be replaced")
...
arena:EnforceBounds()
assert(#created == 16, "fight must not regrow divider")
arena:StartPrepare({})
assert(#created == 31, "next stage restores entire tree divider")
...
arena:EnforceBounds()
assert(#created == 31, "bounds think must not regrow a directly opened divider")
