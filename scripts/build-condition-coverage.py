...
def render(snapshot, mapping):
    ...
    unique = ['# 原生技能专属机制库存（专属机制未实现）', '',
        ...
        '全部1,095个原生定义的设计库存，不是执行方案。每个定义保留G00–G21原始标签、原生标记、中英文机制描述及缺失能力。专属机制全部 **未实现**，包括被动观察、旧版/隐藏子技能和AD变体。普通通用模式不表示专属机制已实现。无描述条目明确保留缺失，不猜造语义。历史复核可能描述已撤回模式，不是当前功能承诺；其中动作时间历史、召唤物数量、属性阈值及已撤回目标条件等仅为文档证据，不进入当前预设。', '',
