...
FAMILIES = {
    'offensive_vector': (rule(use=[NEAR], filters=[condition('distance_lte', value=900), condition('not_invulnerable')]), 'Reviewed ordinary enemy vector mode: select the nearest legal in-range enemy. Native point start or primary unit uses that anchor; runtime continues caster-to-anchor direction by 150 units. Zero-length direction fails. No vector parameters, landing plan or ricochet-hit prediction.'),
    'offensive_unit': (rule(filters=[condition('distance_lte', value=900), condition('not_invulnerable')]), 'Native hostile unit targeting: select a nearby non-invulnerable opponent rather than spending the action without combat context.'),
...
assign('gapclose', '''antimage_blink queenofpain_blink phantom_assassin_phantom_strike riki_blink_strike chaos_knight_reality_rift huskar_life_break spirit_breaker_charge_of_darkness spirit_breaker_nether_strike storm_spirit_ball_lightning morphling_waveform sandking_burrowstrike faceless_void_time_walk void_spirit_astral_step''')
...
def render(snapshot, mapping):
    ...
    lines = ['# 通用技能条件覆盖', '', '<!-- Generated by scripts/build-condition-coverage.py -->', '',
        ...
        '仅实现通用伤害、治疗、保护、增益、基础接近/生命阈值、开关和邻近条件。充能变体观察当前技能，不引用另一个技能ID。普通语义逐ID复核只选择这些通用模板族。通用层数/剩余时间、动作历史及普通归属/召唤物数量条件可由玩家配置，但这里不内置技能专属配方。', '',
    ...
    unique = ['# 原生技能专属机制库存（专属机制未实现）', '',
        '<!-- Generated by scripts/build-condition-coverage.py -->', '',
        '全部1,095个原生定义的设计库存，不是执行方案。每个定义保留G00–G21原始标签、原生标记、中英文机制描述及缺失能力。专属机制全部 **未实现**，包括被动观察、旧版/隐藏子技能和AD变体。普通通用模式不表示专属机制已实现。无描述条目明确保留缺失，不猜造语义。历史复核可能描述已撤回模式，不是当前功能承诺。', '',
