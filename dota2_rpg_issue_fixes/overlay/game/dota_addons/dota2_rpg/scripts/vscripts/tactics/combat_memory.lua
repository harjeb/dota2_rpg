...
function CombatMemory:RecordActionUse(unit, logical_action_id)
    ...
    -- A new handle may reuse an old entindex, but must start with fresh history.
    self.action_use_count[unit] = self.action_use_count[unit] or {}
    local per_unit = self.action_use_count[unit]
...
function CombatMemory:GetActionUseCount(unit, logical_action_id)
    local per_unit = self.action_use_count[unit] or {}
