...
离线检查 **73 / 73 通过**；矩阵覆盖 **439 个技能、505 个预设变体、11,546 个配置组合**，UI 与服务端判断不一致为 0。这不是原生施法成功率；本次未执行 Dota/Workshop 原生验收，仍保留 `native_execution_validated = 0`。

运行全部检查：`python scripts/test-all.py`。需要 Python、Node 和 Lua 命令行运行时，非标准 Lua 路径通过 `LUA_BIN` 指定；后端检查不再因缺少 Lupa 而跳过。

## 当前玩法边界

- 每局 Run 只保存在服务端内存；不使用 Panorama `LocalStorage`、跨局存档、存档码、每日重置或迁移。生命耗尽可点击“再来一次”重试当前关，最终通关可从第一关再战；保留养成、装备、规则和金币并恢复 5 条生命，每次战斗奖励仍只结算一次。
...
- **界面版本 40**：参考 OpenDota 的 64 英雄购买统计与 20 场职业比赛、200 份玩家背包样本，强化 93 套敌方配装，22 级起六格成装；敌方大招先于小技能。白牛 Boss 配双银月、常态魔抗 80%，后期远古再提高生命与双抗。魔晶购买直接升级当前所选英雄，胜败终局增加“再来一次”，并接入用户提供的“团战模拟器”载入画面。数据来源、策略、数值和验证边界见 [RUNTIME_FIXES_V40.md](docs/RUNTIME_FIXES_V40.md)。
