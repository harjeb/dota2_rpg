...
这是一个 Lua + Panorama 的 Dota 2 自定义游戏 addon。进图后选择**单机闯关**或**天梯对战**：闯关保留招募、成长和三十关流程；天梯使用五名满级英雄、99999 金币准备、七场异步对战与独立积分，并支持站位保存、随机敌队测试和 JSON 配置导出。详见 [天梯模式说明](docs/arena-mode.md)。
...
离线检查 **76 / 76 通过**；矩阵覆盖 **439 个技能、505 个预设变体、11,546 个配置组合**，UI 与服务端判断不一致为 0。这不是原生施法成功率；本次未执行 Dota/Workshop 原生验收，仍保留 `native_execution_validated = 0`。

运行全部检查：`python scripts/test-all.py`。需要 Python、Node 和 Lua 命令行运行时，非标准 Lua 路径通过 `LUA_BIN` 指定；后端检查不再因缺少 Lupa 而跳过。

## 单机闯关玩法边界

- 每局 Run 只保存在服务端内存。生命耗尽或最终通关后，点击“再来一次”均从第一关再战；清空旧英雄、养成、装备、规则和金币，恢复新局 1500 金币、2 次免费招募和 5 条生命；每次战斗奖励仍只结算一次。UI96 在技能调试界面新增[条件配置导入／导出](docs/RULE_LIBRARY_UI96.md)：仅将已确认的条件配置收集到本机供备份，新局仍使用默认条件，必须手动导入才应用配置；不保存战斗进度。
- 初始金币为 **1500**；开局拥有 **2 次免费招募选择**。系统不随机赠送英雄，前两次成功选择免费，之后按招募等级固定价格扣款。
- 英雄价格由招募等级决定：1/5/10/15/20/24 级分别为 500/900/1600/2600/4000/5500；品质只决定出现概率和内置魔晶/神杖效果，不乘价格倍率。
- 关卡总经验由全部已拥有英雄平分，包含上阵和待命英雄；升级使用累计经验阈值的相邻差值。时间金币奖励上限为基础金币的 10%。
...
- **界面版本 44**：修复首关胜利切关时 `RealTime` 不存在导致的预加载报错。进入地图后依次后台加载所有后续关卡，开场仍仅同步首关；装备、金币和经验在单个结算框同时显示并一起关闭。日志证据及验证见 [RUNTIME_FIXES_V44.md](docs/RUNTIME_FIXES_V44.md)。
- UI43：关卡掉落移除所有配方卷轴与肉山战旗，当前为 271 个源条目、269 种实际物品；三次掉落判定和新局彻底清空行为不变。见 [RUNTIME_FIXES_V43.md](docs/RUNTIME_FIXES_V43.md)。
- UI42（掉落范围已由 UI43 调整）：关卡掉落扩展为普通装备、散件、卷轴、消耗品、当前中立装备／附魔与肉山奖励，共 351 个源条目（349 种实际物品）；每关最多三件，不包含废弃／内部／测试定义。“再来一次”彻底清空旧进度，指挥官也重新创建，客户端草稿与库存一并清除。见 [RUNTIME_FIXES_V42.md](docs/RUNTIME_FIXES_V42.md)。
- UI41：启动仅同步加载第一关与通用必需资源（3 种单位、4 种物品），后台预加载（UI44 起入场即排队全部后续关卡）；资源未就绪禁止开战，失败可重试。胜败终局的“再来一次”均回到第一关。实现、日志与验证边界见 [RUNTIME_FIXES_V41.md](docs/RUNTIME_FIXES_V41.md)。
- UI40：参考 OpenDota 的 64 英雄购买统计与 20 场职业比赛、200 份玩家背包样本，强化 93 套敌方配装，22 级起六格成装；敌方大招先于小技能。白牛 Boss 配双银月、常态魔抗 80%，后期远古再提高生命与双抗。魔晶购买直接升级当前所选英雄，胜败终局增加“再来一次”，并接入用户提供的“团战模拟器”载入画面。数据来源、策略、数值和验证边界见 [RUNTIME_FIXES_V40.md](docs/RUNTIME_FIXES_V40.md)。
- UI39：阿哈利姆魔晶开局即可购买；UI40 对魔晶增加订单拦截，避免先被小精灵消耗。原价 1400、每英雄一次和原生升级效果保留，一秒补货；其他物品继续使用原版购买流程。
...
- 敌方英雄的等级和原版装备由 `scripts/data/levels.kv` 配置并在生成时实际装备；敌我双方共用修订版战术执行引擎。修改等级或 [配装档案](data/enemy_equipment_profiles.json) 后，运行 `python scripts/author-enemy-equipment.py` 更新装备，`python scripts/author-late-campaign-balance.py --verify-native` 更新 Boss / 远古数值，再运行 `python scripts/export-level-configuration.py` 更新 [关卡配置表](exports/level_configuration_current/当前关卡配置.xlsx)。研究快照可用 `python scripts/research-enemy-equipment.py fetch` 从公开接口重新采集；生成装备不联网。
