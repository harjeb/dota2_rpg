...
- **界面版本 43**：关卡掉落移除所有配方卷轴与肉山战旗，当前为 271 个源条目、269 种实际物品；三次掉落判定和新局彻底清空行为不变。见 [RUNTIME_FIXES_V43.md](docs/RUNTIME_FIXES_V43.md)。
...
- UI41：启动仅同步加载第一关与通用必需资源（3 种单位、4 种物品），每关提前在后台加载下一关；资源未就绪禁止开战，失败可重试。胜败终局的“再来一次”均回到第一关。实现、日志与验证边界见 [RUNTIME_FIXES_V41.md](docs/RUNTIME_FIXES_V41.md)。
